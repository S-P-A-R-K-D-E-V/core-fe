...
export default function ReportBreakEvenView() {
  ...
  const [cogsRatio, setCogsRatio] = useState(0.6);
  const [data, setData] = useState<IBreakEvenAnalysis | null>(null);

  const fetchData = useCallback(async () => {
    try {
      const result = await getBreakEvenAnalysis({
        period,
        targetDate,
        assumedCogsRatio: cogsRatio,
      });
      setData(result);
    } catch (error) {
      console.error(error);
  ...
  }, [period, targetDate, cogsRatio, enqueueSnackbar]);
    ...
    <RoleBasedGuard hasContent roles={['Admin']}>
      ...
      <Card sx={{ mb: 3 }}>
        <CardContent>
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={3} alignItems={{ md: 'center' }}>
            ...
            <Box sx={{ minWidth: 280, px: 1 }}>
              <Typography variant="caption" color="text.secondary">
                %Giá vốn ước tính trên doanh thu: {fPercent(cogsRatio * 100)}
              </Typography>
              <Slider
                value={cogsRatio}
                onChange={(_, value) => setCogsRatio(value as number)}
                min={0}
                max={0.95}
                step={0.05}
              />
          ...
          <Typography variant="caption" color="text.secondary">
            Hệ thống chưa lưu giá vốn từng sản phẩm nên %giá vốn cần nhập tay theo ước tính thực tế của cửa hàng.
          </Typography>
        </CardContent>
      </Card>
